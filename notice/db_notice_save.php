<?php
    ...
    $queryMax ="select max(no)+1 as maxno from h_notice "; /*파일명이 중복일시 삭제시에 오류가 생길수 있으므로 파일명에 게시글 번호를 부여해주려한다. */
    $resultMax = mysqli_query($connect, $queryMax);
    $rowMax = mysqli_fetch_array($resultMax);
    $maxNo = $rowMax["maxno"];

    $attach_dir =$_SERVER["DOCUMENT_ROOT"]."/file_room/notice/"; /*$_SERVER["DOCUMENT_ROOT"]의 뜻은 apache까지의 경로다. = //C:Apache24/htdocs 란 말과 같다. */
