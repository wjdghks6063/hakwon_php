...
/**********************************************************************/
	...
	$query ="SELECT a.no, a.attach, a.title, b.price_num, b.price, b.reg_date, a.shop_name, a.stuff_number, b.price_code, b.orderno, b.id, c.point FROM h_shop a, h_basket b, h_member c ".
...
//			alert("배열이다");
	...
	/*	//이번 경우에 배열에서 체크한것만 인식하기 떄문에 체크박스의 값을 넘겨줄 필요가 없어졌다.
	...
	*/
		...
		//	var check = notiArr.t_check[index].value; //배열인 경우 번호값을 넣어줘서 몇번쨰 자리의 값인지 찾아준다. //이번 경우에 배열에서 체크한것만 인식하기 떄문에 체크박스의 값을 넘겨줄 필요가 없어졌다.
			var orderno = notiArr.t_orderno[index].value;
			var price_num = notiArr.t_price_num[index].value;

			//테스트
		//	alert("상품 리스트 번호 : "+orderno);
		//	alert("체크 여부 : "+check); //이번 경우에 배열에서 체크한것만 인식하기 떄문에 체크박스의 값을 넘겨줄 필요가 없어졌다.
		//	alert("상품 수량 : "+price_num);
			
		//	hitForm.t_no.value=no;

		//	price_form.t_check.value= check; //이번 경우에 배열에서 체크한것만 인식하기 떄문에 체크박스의 값을 넘겨줄 필요가 없어졌다.
			price_form.t_orderno.value= orderno;
		...
		*/
		...
		//	var check = notiArr.t_check.value; //이번 경우에 배열에서 체크한것만 인식하기 떄문에 체크박스의 값을 넘겨줄 필요가 없어졌다.
		...
		//	alert(check); //이번 경우에 배열에서 체크한것만 인식하기 떄문에 체크박스의 값을 넘겨줄 필요가 없어졌다.
		//	alert(orderno);
		//	alert(price_num);
		}
...
<form name ="pageForm">
	<input type="hidden" name="t_page">
		...
		<div class="notice">
			...
			<!-- table start-->
		<div style="float: right; padding: 20px 50px; font: size 14px;">내 포인트 : 0 \</div>
			<div class="table-box">
				<table class="table">
